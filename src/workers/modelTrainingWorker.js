...
function makeContext(products, users) {
    ...
    return {
        ...
        // note: spelling corrected for future use
...
function encodeProduct(product, context) {
    ...
    // categoriesIndex and colorsIndex are plain objects, not functions
...
function createTrainingData(context){
    ...
    return {
        ...
        // tamaho = userVector + productVector
    }
}
...
async function configureNeuralNetAndTrain(trainData){
    ...
    model.compile({
        optimizer: tf.train.adam(0.01), // Taxa de aprendizado: quão rápido o modelo ajusta seus pesos
        loss: 'binaryCrossentropy', // Função de perda para classificação binária (comprou ou não comprou)
        metrics: ['accuracy'] // Métrica para avaliar o desempenho durante o treinamento
    });
...
function recommend({ user }) {
    ...
    // 1. Converter o usuario fornecido no vetor de features codificadas
    // (preco ignorada, idade normalizada, categorias ignoradas, cores ignoradas)
    // Isso transforma as informacoes do usuario no mesmo formato numerico que foi usado
    //  para treinar o modelo, garantindo que ele possa processar corretamente os dados de entrada.
    
    const userVector = encodeUser(user, _globalCtx).dataSync();

    //OBSERVACAO: Em aplicacoes reais, armazene todos os vetores de produtos em um banco de dados
    // vetorial (como Postgres, Neo4j ou Pinecone)
    //Consulta: Encontre os 200 produtos mais proximos do vetor do usuario
    // Execute _model.predict() apenas nesses produtos para obter as recomendacoes, 
    // em vez de processar toda a base de produtos (o que seria ineficiente).

    // 2. Criar pares de entrada: para cada produto, concatena o vetor do usuario
    // com o vetor codificado do produto.
    // Por que? O modelo preve o "score de compatibilidade"
    // para cada par (usuario, produto).
    const inputs = context.productVectors.map(({vector}) => {
        return [...userVector,...vector]
    });

    // 3. Converter todos os pares (usuario, produto) em um unico tensor.
    // Formato: [numProdutos, inputDim]

    const inputTensor = tf.tensor2d(inputs);

    // Rodar a rede neural treinada em todos os pares (usuario, produto) de uma vez.
    // O resultado eh uma pontuacao para cada produto entre 0 e 1.

    // Quanto maior, maior a probabilidade do usuario querer aquele produto.

    const predictions = _model.predict(inputTensor);

    // 5. Extrair as pontuacoes para um array JS normal.
    const scores = predictions.dataSync();

    const reommendations = context.productVectors.map((item, index) => {
        return {
            ...item.meta,
            name: item.name,
            score: scores[index] //previsao do modelo para este produto

        }
    })

    const sortedItems = reommendations
                        .sort((a,b) => b.score - a.score);
    
    // 6. Enviar a lista ordenada de produtos recomendados 
